...
class Backtester:
    ...
    def execute_trades(
        ...
        # Cooldown adaptatif selon le timeframe (plus court pour la crypto)
        if "1w" in self.timeframe:
            ...
            min_bars_between_trades = 8   # intraday court
        ...
        # Seuil minimum sur le retour prédit (plus bas pour crypto)
        ...
        for i in range(len(predictions)):
            ...
            # Seuil minimum par timeframe (plus faible pour crypto)
            if "1w" in self.timeframe or "1d" in self.timeframe:
                ...
                min_amplitude = 0.005   # 0,5% intraday court
            ...
            if position is None and not too_soon:
                if use_model_filters:
                    ...
                    if cls < 0:
                        self.equity_curve.append(capital)
                        continue
                    # 2) Retour attendu minimum
                    if np.isfinite(pred_ret) and pred_ret < min_predicted_return:
                        self.equity_curve.append(capital)
                        continue

                # Sinon : on trade dès qu'on est flat + ATR OK
    ...
    def print_report(self):
        ...
        print(
            f"Gagnants: {metrics['winning_trades']} | "
            f"Perdants: {metrics['losing_trades']}"
        )
        print(
            f"TP1: {metrics['tp1_hits']} | "
            f"TP2: {metrics['tp2_hits']} | "
            f"SL: {metrics['sl_hits']}"
        )
